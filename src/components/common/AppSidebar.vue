<template>
  <aside class="fixed top-0 left-0 h-full z-50 flex flex-col transition-transform duration-300" :class="[
    ...
    <div class="flex items-center gap-3 px-5 py-[18px]" style="border-bottom:1px solid #242d3e">
      ...
      <div class="min-w-0">
        <div class="font-display font-bold text-[15px] text-white leading-tight truncate">
          {{ auth.stationName }}
        </div>

        <div class="text-[10px] text-[#5a6a82] uppercase tracking-widest">
          HP Fuel Station
        </div>
      </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-3 px-3">
      ...
      <div class="mb-3">
        ...
        <NavItem to="/sales" :icon="Fuel" label="Petrol Sales">
          <template #badge>
            <span class="ml-auto text-[10px] font-bold bg-[#f59e0b]/20 text-[#f59e0b] px-1.5 py-0.5 rounded-full">
              Daily
            </span>
          </template>
        </NavItem>

        <NavItem to="/sales/new" :icon="PlusCircle" label="New Sale Entry" />

        <NavItem to="/stock" :icon="Package" label="Stock Summary" />

        <NavItem to="/meter" :icon="BarChart3" label="Meter Readings" />
      </div>
    ...
    <div class="px-3 py-3" style="border-top:1px solid #242d3e">
      <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer hover:bg-[#161b24] transition-colors">
        <!-- AVATAR -->
        <div
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="text-[11px] text-[#5a6a82] capitalize">
            {{ auth.role }}
          </div>
        </div>

        <!-- LOGOUT -->
        <button @click="handleLogout" class="text-[#5a6a82] hover:text-[#ef4444] transition-colors" title="Logout">
          <LogOut :size="17" />
        </button>
      </div>
    </div>
...
import { LayoutDashboard, Fuel,  PlusCircle,  Package,  BarChart3,  CreditCard,  Wallet,  Users,  Clock3,  FileText,  
  Settings,  LogOut } from 'lucide-vue-next'
